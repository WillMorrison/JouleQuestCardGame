...
func (p Params) Valid() error {
	...
	if p.BatteryBuildCost <= p.InitialCash {
		errs = append(errs, fmt.Errorf("battery build cost (%d) should be greater than initial money (%d)", p.BatteryBuildCost, p.InitialCash))
	}
	if p.RenewableBuildCost <= p.InitialCash {
		errs = append(errs, fmt.Errorf("renewable build cost (%d) should be greater than initial money (%d)", p.RenewableBuildCost, p.InitialCash))
	}
	if p.FossilBuildCost <= p.InitialCash {
		errs = append(errs, fmt.Errorf("fossil build cost (%d) should be greater than initial money (%d)", p.FossilBuildCost, p.InitialCash))
	}
